...
func UpdatePackages() *ShellCommand {
	return And("yum update", "DEBIAN_FRONTEND=noninteractive yum upgrade -y")
}

// Upgrade the package cache and update the installed packages (using apt).
func UpdatePackagesOmitError() *ShellCommand {
	return Or("yum update", "DEBIAN_FRONTEND=noninteractive yum upgrade -y")
}
...
func UpdateSelectedRepoPackages(repoConfigPath string) *ShellCommand {
	return &ShellCommand{
		Command: fmt.Sprintf(
			`yum update -o Dir::Etc::sourcelist="sources.list.d/%s" -o Dir::Etc::sourceparts="-" -o APT::Get::List-Cleanup="0"`,
			repoConfigPath,
		),
	}
}
...
func InstallPackagesWithoutForce(pkg string, pkgs ...string) *ShellCommand {
	return &ShellCommand{
		Command: fmt.Sprintf("DEBIAN_FRONTEND=noninteractive yum install %s %s", pkg, strings.Join(pkgs, " ")),
	}
}
