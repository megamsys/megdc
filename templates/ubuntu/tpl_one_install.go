...
func (m *UbuntuOneInstallTemplate) Render(pkg urknall.Package) {
	...
	pkg.AddCommands("requires",
		...
		Shell("sudo -H -u oneadmin bash -c 'echo "+Slash+"'TM_MAD=ssh"+Slash+"' > /tmp/ds_tm_mad'"),
		Shell("sudo -H -u oneadmin bash -c 'onedatastore update 0 /tmp/ds_tm_mad'"),
		Shell("sudo -H -u oneadmin bash -c 'onedatastore update 1 /tmp/ds_tm_mad'"),
		Shell("sudo -H -u oneadmin bash -c 'onedatastore update 2 /tmp/ds_tm_mad'"),
